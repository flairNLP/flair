...
def test_load_ag_news_data(tasks_base_path):
    ...
    assert len(list(corpus.train())) == 10
    assert len(list(corpus.dev())) == 10
    assert len(list(corpus.test())) == 10
...
def test_load_sequence_labeling_data(tasks_base_path):
    ...
    assert len(list(corpus.train())) == 6
...
def test_load_germeval_data(tasks_base_path):
    ...
    assert len(list(corpus.train())) == 2
    assert len(list(corpus.dev())) == 1
    assert len(list(corpus.test())) == 1
...
def test_load_ud_english_data(tasks_base_path):
    ...
    assert len(list(corpus.train())) == 6
    assert len(list(corpus.test())) == 4
    assert len(list(corpus.dev())) == 2
...
def test_multi_corpus(tasks_base_path):
    ...
    assert len(list(corpus.train())) == 8
    assert len(list(corpus.dev())) == 2
    assert len(list(corpus.test())) == 2
