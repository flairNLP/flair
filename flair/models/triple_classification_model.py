...
import flair.nn
from flair.data import Corpus, Sentence, TextTriple, _iter_dataset
...
class TextTripleClassifier(flair.nn.DefaultClassifier[TextTriple, TextTriple]):
    ...
    def __init__(
        ...
        **classifierargs,
        ...
        """Initializes a TextTripleClassifier.
    ...
    def get_used_tokens(
        ...
        for sentence_pair in _iter_dataset(corpus.get_all_sentences()):
            ...
            yield [t.text for t in sentence_pair.third]
            yield [t.text for t in sentence_pair.third.left_context(context_length, respect_document_boundaries)]
            yield [t.text for t in sentence_pair.third.right_context(context_length, respect_document_boundaries)]
