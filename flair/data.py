...
class Dictionary:
    ...
    def __setstate__(self, d):
        ...
        if "add_unk" not in self.__dict__.keys():
            self.__dict__["add_unk"] = (
                True if b"<unk>" in self.__dict__["idx2item"] else False
            )
    ...
    def load(cls, name: str):
        ...
        if name == "chars-lemmatizer" or name == "common-chars-lemmatizer":
            char_dict = cached_path(
                f"{hu_path}/common_characters_lemmatizer", cache_dir="datasets"
            )
            return Dictionary.load_from_file(char_dict)
    ...
    def __eq__(self, o: object) -> bool:
        if not isinstance(o, Dictionary):
            return False
        return (
            self.item2idx == o.item2idx
            and self.idx2item == o.idx2item
            and self.add_unk == o.add_unk
        )
...
class EntityLinkingLabel(Label):
    ...
    def __repr__(self):
        if self.additional_ids is None:
            return f"{self.database}:{self._value} {self.concept_name} [{self.span.id_text}] ({round(self._score, 2)})"
        else:
            return f"{self.database}:{self._value} ({', '.join(self.additional_ids)}) {self.concept_name} [{self.span.id_text}] ({round(self._score, 2)})"
...
class DataPoint:
    ...
    def add_complex_label(self, typename: str, label: Label):
        ...
        if (
            typename in self.annotation_layers
            and label in self.annotation_layers[typename]
        ):
            return self
...
class Span(DataPoint):
    ...
    def add_tag(self, tag_type: str, tag_value: str, confidence=1.0):
        ...
        self.tokens[0].sentence.add_complex_label(
            tag_type, SpanLabel(self, value=tag_value, score=confidence)
        )
...
class Sentence(DataPoint):
        ...
        for token in self:
            ...
            if (starts_new_span or not in_span) and len(current_span) > 0:
                ...
                if span_score > min_score:
                    ...
                    value = sorted(tags.items(), key=lambda k_v: k_v[1], reverse=True)[
                        0
                    ][0]
    ...
    def left_context(
        self, context_length: int, respect_document_boundaries: bool = True
    ):
        sentence = self
    ...
    def is_context_set(self) -> bool:
        ...
        """
        return (
            self._previous_sentence is not None or self._position_in_dataset is not None
        )
    ...
    def get_labels(self, label_type: str = None):
        ...
        if label_type in set().union(
            *(token.annotation_layers.keys() for token in self)
        ):
            return [
                SpanLabel(
                    Span([token]),
                    token.get_tag(label_type).value,
                    token.get_tag(label_type).score,
                )
                for token in self
...
class Corpus:
    def __init__(
        ...
        if (
            test is None
            and sample_missing_splits
            and train
            and not sample_missing_splits == "only_dev"
        ):
        ...
        if (
            ...
            and sample_missing_splits
            and train
            and not sample_missing_splits == "only_test"
        ):
            train_length = _len_dataset(train)
    ...
    def make_label_dictionary(self, label_type: str, min_count: int = -1) -> Dictionary:
        ...
        log.info(
            f"{erfasst_count} instances in dict, {unked_count} instances are UNK'ed"
        )
        log.info(
            f"Most commonly observed '{label_type}'-labels are {label_occurrence.most_common(20)}"
        )
...
class MultiCorpus(Corpus):
    ...
    def __str__(self):
        ...
        output += "\n - ".join(
            [
                f"{type(corpus).__name__} {str(corpus)} - {corpus.name}"
                for corpus in self.corpora
            ]
        )
        return output
