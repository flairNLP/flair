...
class ModelTrainer:
    ...
    def train(self,
        ...
        number_of_iterations = ((len(train_data) // mini_batch_size) * max_epochs) + max_epochs + 1

        scheduler = OneCycle(optimizer, learning_rate, number_of_iterations, min_lr_magnitude=min_lr_magnitude)
            ...
            for epoch in range(0 + self.epoch, max_epochs + self.epoch):
                ...
                if not param_selection_mode:
                    with open(loss_txt, 'a') as f:
                        ...
                        f.write(
                            f'{epoch}\t{datetime.datetime.now():%H:%M:%S}\t0\t{learning_rate:.4f}\t'
                ...
                current_score = train_loss if train_with_dev else dev_score
