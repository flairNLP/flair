...
class ModelTrainer(Pluggable):
    ...
    def train_custom(
        ...
        **kwargs,
        ...
        param decoder_learning_rate (Optional[float]): Optional, if set, the decoder is trained with a separate learning rate
        param mini_batch_size (int): Size of mini-batches during training
        param eval_batch_size (int): Size of mini-batches during evaluation
        param mini_batch_chunk_size (int): If mini-batches are larger than this number, they get broken down into chunks of
            this size for processing purposes
        param max_epochs (int): Maximum number of epochs to train. Terminates training if this number is surpassed.
        param optimizer: The optimizer to use (typically SGD or Adam)
        param train_with_dev (bool): If True, the data from dev split is added to the training data
        param train_with_test (bool): If True, the data from test split is added to the training data
        param main_evaluation_metric: The metric to optimize (often micro-average or macro-average F1-score, or accuracy)
        param monitor_test (bool): If True, test data is evaluated at end of each epoch
        param monitor_train_sample: Set this to evaluate on a sample of the train data at the end of each epoch.
        ...
        param use_final_model_for_eval (bool): If True, the final model is used for the final evaluation. If False, the
            model from the best epoch as determined by main_evaluation_metric is used for the final evaluation.
        param gold_label_dictionary_for_eval: Set to force evaluation to use a particular label dictionary
        param exclude_labels: Optionally define a list of labels to exclude from the evaluation
        param sampler: You can pass a data sampler here for special sampling of data.
        param shuffle: If True, data is shuffled during training
        param shuffle_first_epoch: If True, data is shuffled during the first epoch of training
        param embeddings_storage_mode: One of 'none' (all embeddings are deleted and freshly recomputed),
            'cpu' (embeddings stored on CPU) or 'gpu' (embeddings stored on GPU)
        param epoch: The starting epoch (normally 0 but could be higher if you continue training model)
        param save_final_model: If True, the final model is saved at the end of training.
        param save_optimizer_state (bool): If True, the optimizer state is saved alongside the model
        ...
        param create_file_logs (bool): If True, logging output is written to a file
        param create_loss_file (bool): If True, a loss file logging output is created
        param write_weights (bool): If True, write weights to weights.txt on each batch logging event.
        param plugins: Any additional plugins you want to pass to the trainer
        param **kwargs: Additional arguments, for instance for the optimizer

        return dict: A dictionary with at least the key "test_score" containing the final evaluation score. Some plugins
