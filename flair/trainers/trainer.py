...
class ModelTrainer:
        ...
        try:
            ...
            for epoch in range(0, max_epochs):
                ...
                for batch_no, batch in enumerate(batches):
                    ...
                    ## called on mode, directly returns loss
                ...
                if not train_with_dev:
                    dev_score, dev_metric = ModelTrainer.evaluate(self.model, self.corpus.dev, base_path,
                                                                  evaluation_method=evaluation_method,
                                                                  embeddings_in_memory=embeddings_in_memory)

                test_score, test_metric = ModelTrainer.evaluate(self.model, self.corpus.test, base_path,
                                                                evaluation_method=evaluation_method,
                                                                embeddings_in_memory=embeddings_in_memory)
    ...
    @staticmethod
    def evaluate(model, evaluation: List[Sentence], out_path=None, evaluation_method: str = 'F1',
                 eval_batch_size: int = 32,
                 embeddings_in_memory: bool = True):

        batch_no: int = 0
        batches = [evaluation[x:x + eval_batch_size] for x in range(0, len(evaluation), eval_batch_size)]
        ...
        for batch in batches:
            ...
            scores, tag_seq = model._predict_scores_batch(batch)
            ...
            for (token, score, predicted_id) in zip(all_tokens, scores, predicted_ids):
                ...
                predicted_value = model.tag_dictionary.get_item_for_index(predicted_id)
            ...
            for sentence in batch:
                ...
                gold_tags = [str(tag) for tag in sentence.get_spans(model.tag_type)]
            ...
            if not embeddings_in_memory:
                ModelTrainer.clear_embeddings_in_batch(batch)
    ...
    @staticmethod
    def clear_embeddings_in_batch(cls, batch: List[Sentence]):
        for sentence in batch:
            for token in sentence.tokens:
