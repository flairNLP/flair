...
from flair.datasets.base import ConcatPairDataset, SubsetPairDataset
from flair.samplers import SimilarityBatchSampler
...
class SimilarityModelTrainer:
    ...
    def train(
        ...
        data_loader = SimilarityBatchSampler,
        ...
        shuffle: bool = True,
        squashing_function = lambda x: np.floor(np.sqrt(x)),
        similarity_pair_threshold: int = 2,
        topk: int = 40,
        ...
        **kwargs,
        ...
        if train_with_dev:
            train_data = ConcatPairDataset([self.corpus.train, self.corpus.dev])
        ...
        try:
            ...
            for self.epoch in range(self.epoch + 1, max_epochs + 1):
                ...
                batch_loader = data_loader(train_data,
                                           mini_batch_size,
                                           shuffle,
                                           squashing_function,
                                           similarity_pair_threshold,
                                           topk)
                ...
                for batch_no, batch in enumerate(batch_loader):
                    ...
                    batch_steps = [batch]
                    # TODO: this needs to be re-formatted as batch needs to be
                    # split in different way
                    ...
                    # TODO: uncomment when store_embeddings handles this special batch format
    ...
    def load_checkpoint(cls, checkpoint: Union[Path, str], corpus: Corpus):
        model: SimilarityModelTrainer = torch.load(checkpoint, map_location=flair.device)
        model.corpus = corpus
        return model
