...
class UP_PORTUGUESE(ColumnCorpus):
    def __init__(
            self,
            base_path: Union[str, Path] = None,
            in_memory: bool = True,
        ...
        """
        Initialize the Portuguese dataset from the Universal Propositions Bank, comming from that webpage:
        https://github.com/System-T/UniversalPropositions/tree/master/UP_Portuguese-Bosque
        ...
        # download data if necessary
        up_pt_path = "https://raw.githubusercontent.com/System-T/UniversalPropositions/master/UP_Portuguese-Bosque/"
        cached_path(f"{up_pt_path}pt_bosque-up-train.conllu", Path("datasets") / dataset_name)
        cached_path(f"{up_pt_path}pt_bosque-up-dev.conllu", Path("datasets") / dataset_name)
        cached_path(f"{up_pt_path}pt_bosque-up-test.conllu", Path("datasets") / dataset_name)

        super(UP_PORTUGUESE, self).__init__(
            data_folder,
            columns,
            encoding="utf-8",
            train_file="pt_bosque-up-train.conllu",
            test_file="pt_bosque-up-test.conllu",
            dev_file="pt_bosque-up-dev.conllu",
            in_memory=in_memory,
            document_separator_token=None if not document_as_sequence else "-DOCSTART-",
            comment_symbol="#",
        )