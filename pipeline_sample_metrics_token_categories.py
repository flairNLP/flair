...
def relabel_category(dataset, tag_column = 'ner', prediction_bio_column = 'predicted_bio', metric = 'confidence',threshold = 0.7, direction='left', category_id='2'):
    ...
    for sent in dataset:
        ...
        for token in sent: 
            if (token.get_label(tag_column_bio).value == token.get_label(prediction_bio_column).value) == category_conditions[category_id][0] and (token.get_label(tag_column_bio).value == 'O') == category_conditions[category_id][1]: # if incorrect prediction
                flag=True
                if direction == 'left':
                    if token.get_metric(metric) < float(threshold): # rule for masking
                        token.set_label(tag_column_bio, token.get_label(prediction_bio_column).value)
                        tokens_changed += 1
        ...
        bioes_tags = [token.get_label(tag_column_bio).value for token in sent]
        ...
        for span_indices, score, label in predicted_spans:
            ...
            if label != "O":
                span.set_label(tag_column, value=label, score=score)
    return tokens_changed, tokens_changed_additionally

def mask_category(dataset, tag_column = 'ner', prediction_bio_column = 'predicted_bio', metric = 'confidence',threshold = 0.7, direction='left', category_id='1'):
    tag_column_bio = f'{tag_column}_bio'
    for sent in dataset:
        # debug
        print(sent.text)
        ...
        for token in sent: 
            if (token.get_label(tag_column_bio).value == token.get_label(prediction_bio_column).value) == category_conditions[category_id][0] and (token.get_label(tag_column_bio).value == 'O') == category_conditions[category_id][1]: # if incorrect prediction
                if direction == 'left':
                    if token.get_metric(metric) < float(threshold): # rule for masking
                        flag=True
                        token.set_label(tag_column_bio, 'S-MASK')
                else:
                    if token.get_metric(metric) > float(threshold): # rule for masking
                        flag=True
                        token.set_label(tag_column_bio, 'S-MASK')
...
def add_bioes_ner_tags(dataset, tag_column = 'ner'):
    # add a new column to dataset, which contains token-level tags for NER in BIOES (observed label)
    for sent in dataset:
        for token in sent:
            token.add_label(f'{tag_column}_bio', 'O')
        for span in sent.get_spans(tag_column):
            if len(span.tokens) == 1:
                span[0].set_label(f'{tag_column}_bio', f'S-{span.get_label(tag_column).value}')
            else:
                span[0].set_label(f'{tag_column}_bio', f'B-{span.get_label(tag_column).value}')
                span[-1].set_label(f'{tag_column}_bio', f'E-{span.get_label(tag_column).value}')
                for i in range(1, len(span) - 1):
                    span[i].set_label(f'{tag_column}_bio', f'I-{span.get_label(tag_column).value}')

def run_experiment(seed, config, category_config, output_path, data_path, train_filename, dev_filename, test_filename, tag_type, initialize_decoders_lr, num_epochs_decoder_init, only_clean_flag, pre_train_clean_flag, post_train_clean_flag, category_id):
    ...
    calculate_f1_between_columns(conll_corpus.train, 'ner_clean','ner', label_dictionary=tag_dictionary)
    #input()

    add_bioes_ner_tags(conll_corpus.train, tag_column = 'ner')
    output_bio_dataset(conll_corpus.train, tag_column = 'ner_bio', filename = f'{output_path}/noise_crowd_backup.train')
    ...
    if category_id != '0':
        epoch_change = int(category_config["epoch_change"])
    else:
    ...
    if config['parameters']['seq_tagger_mode'] == 'standard':
        ...
        if only_clean_flag or pre_train_clean_flag: # use clean validation data for additional fine-tuning
            ...
            tagger = SequenceTagger(
                ...
                calculate_sample_metrics=True,
                metrics_mode = metrics_mode,
                metrics_save_list = [category_config['metric']]
        ...
        if metrics_mode == 'epoch_end':
            ...
            tagger.evaluate(
                conll_corpus.test, gold_label_type="ner", out_path=output_path_training + os.sep + "train_sample_0.tsv", **kwargs
            )
            ...
            kwargs['final_train_eval'] = True
            tagger.evaluate(
                conll_corpus.dev, gold_label_type="ner", out_path=output_path_training + os.sep + "train_sample_0.tsv", **kwargs
            )
    ...
    if only_clean_flag:
        ...
        if category_id == '0':
            ...
            if category_config['modification'] == 'relabel': # this will be only category2 for now.
                tokens_changed, tokens_changed_additionally = relabel_category(conll_corpus.train, tag_column = tag_type, prediction_bio_column = 'predicted_bio',metric = category_config["metric"], threshold = float(category_config["threshold"]), direction=category_config["direction"], category_id=category_id)
                print('number of tokens changed:', tokens_changed)
                print('number of consecutive tokens changed:', tokens_changed_additionally)
                score, detailed_result = calculate_f1_between_columns(conll_corpus.train, 'ner','ner_clean', label_dictionary=tag_dictionary)
                noise_f1 = score[('micro avg', 'f1-score')]
                
                with open(f'{output_path_training}/noise_f1.txt', 'w') as f:
                    f.write(f'{noise_f1}\n')
                    f.write(detailed_result)
            elif category_config['modification'] == 'mask':
                mask_category(conll_corpus.train, tag_column = tag_type, prediction_bio_column = 'predicted_bio', metric = category_config["metric"], threshold = float(category_config["threshold"]), direction=category_config["direction"], category_id=category_id)
                score, detailed_result = calculate_f1_between_columns(conll_corpus.train, 'ner','ner_clean', label_dictionary=tag_dictionary)
                noise_f1 = score[('micro avg', 'f1-score')]
            ...
            output_bio_dataset(conll_corpus.train, tag_column = 'ner_bio', filename = f'{output_path_training}/noise_crowd_relabeled.train')
            ...
            if config["parameters"]["model_reinit"] or config["parameters"]["seq_tagger_mode"] == 'EE':
                ...
                if category_id != 'O' and category_config['modification'] == 'mask':
...
def main():
    ...
    if not config["parameters"]['modify_category1']:
        relabel_category1_flag = False
    else:
        relabel_category1_flag = True
        category_config = config["parameters"]['modify_category1']
        category_id = '1'

    if not config["parameters"]['modify_category2']:
        relabel_category2_flag = False
    else:
        relabel_category2_flag = True
        category_config = config["parameters"]['modify_category2']
        category_id='2'

    if not config["parameters"]['modify_category4']:
        relabel_category2_flag = False
    else:
        category_config = config["parameters"]['modify_category4']
        category_id='4'

    if not config["parameters"]['modify_category3']:
        relabel_category2_flag = False
    else:
    ...
    experiment_path = config["paths"]["resources_path"] + "category"+category_id + os.sep + f"{seq_tagger_mode}_{category_config['metric']}"+ os.sep + category_config['f_type'] + os.sep + category_config['modification'] + os.sep
    ...
    for corpus_name in corpora:
        ...
        for seed in seeds:
            score = run_experiment(seed, config, category_config, output_path, data_path, train_filename,dev_filename, test_filename, tag_type, initialize_decoders_lr, num_epochs_decoder_init, only_clean_flag, pre_train_clean_flag, post_train_clean_flag, category_id)
